...
#' @title Efficientdet infer dataloader
#'
...
#' @export
efficientdet_infer_dl <- function(dataset, batch_tfms = NULL, ...) {
  ...
  do.call(icevision()$efficientdet$infer_dl, args)
...
#' @return None
#' @export
efficientdet_predict_dl <- function(model, infer_dl, show_pbar = TRUE) {

  icevision()$efficientdet$predict_dl(
    model = model,
    infer_dl = infer_dl,
    show_pbar = show_pbar
  )
